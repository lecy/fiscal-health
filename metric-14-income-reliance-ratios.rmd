...
Tax data is available for full 990 filers only, so this metric does not describe any organizations with Gross receipts < $200,000 and
...
core2 %>%
  ...
  stargazer( type = s.type, 
             ...
             summary.stat = c("min","p25","median",
                              "mean","p75","max", "sd"),
             covariate.labels = c("Donation Reliance Ratio", "Age", 
...
What proportion of orgs have Donation Reliance Ratios equal to zero?
...
In the sample, `r 100*round(prop.zero,2)` percent of the organizations have Donation Reliance Ratios equal to zero, meaning they have no donation revenues. These organizations are dropped from subsequent graphs to keep the visualizations clean. The interpretation of the graphics should be the distributions of Donation Reliance Ratios for organizations that have positive or negative values.  
...
#Earned Income Reliance Ratio Analysis
...
# summary( core$earned_income_ratio )
```
...
core2$earned_income_ratio[ core2$earned_income_ratio > x.95 ] <- x.95
```
...
## `r variable.label2` Density 

```{r, fig.height=4}
min.x <- min( core2$earned_income_ratio, na.rm=T )
...
## `r variable.label2` by NTEE Major Code
...
## `r variable.label2` by Nonprofit Size (Expenses)
...
## `r variable.label2` by Nonprofit Size (Revenue)
...
## `r variable.label2` by Nonprofit Size (Net Assets)
...
## `r variable.label2` by Nonprofit Age
...
# Investment Income Reliance Ratio Analysis
...
# summary( core$investment_income_ratio )
```
...
core2$investment_income_ratio[ core2$investment_income_ratio > x.95 ] <- x.95
```
...
```{r}
prop.zero <- mean( core2$investment_income_ratio == 0, na.rm=T )
```


In the sample, `r 100*round(prop.zero,2)` percent of the organizations have Investment Income Reliance Ratios equal to zero, meaning they have no earned income. These organizations are dropped from subsequent graphs to keep the visualizations clean. The interpretation of the graphics should be the distributions of Investment Income Reliance Ratios for organizations that have positive or negative values.  
...
```{r, fig.height=4}
min.x <- min( core2$investment_income_ratio, na.rm=T )
...
## `r variable.label3` by NTEE Major Code
...
## `r variable.label3` by Nonprofit Size (Expenses)
...
## `r variable.label3` by Nonprofit Size (Revenue)
...
## `r variable.label3` by Nonprofit Size (Net Assets)
...
## `r variable.label3` by Nonprofit Age
...
# Income Reliance Ratio Analysis

```{r, eval=T}

dat <- dplyr::select( core3, donation_ratio, investment_income_ratio, earned_income_ratio )

core3$income_reliance_ratio <- apply( dat, MARGIN=1, FUN=max )

                     
# summary( core3$income_reliance_ratio )
```
...
x.05 <- quantile( core3$income_reliance_ratio, 0.05, na.rm=T )
x.95 <- quantile( core3$income_reliance_ratio, 0.95, na.rm=T )
...
core3$income_reliance_ratio[ core3$income_reliance_ratio < x.05 ] <- x.05
core3$income_reliance_ratio[ core3$income_reliance_ratio > x.95 ] <- x.95
```
...
## `r variable.label4` Density 

```{r, fig.height=4}
min.x <- min( core3$income_reliance_ratio, na.rm=T )
max.x <- max( core3$income_reliance_ratio, na.rm=T )
...
## `r variable.label4` by NTEE Major Code
...
## `r variable.label4` by Nonprofit Size (Expenses)
...
## `r variable.label4` by Nonprofit Size (Revenue)
...
## `r variable.label4` by Nonprofit Size (Net Assets)
...
## `r variable.label4` by Nonprofit Age
