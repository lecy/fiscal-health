...
$$Quick\:Ratio = \frac{Cash + Short \text-term \: Investments + Current\: Receivables}{Current \: Liabilities} $$

This metric indicates how well an organization can cover its liabilities with its readily available cash. When an organization has a quick ratio of 1, its quick assets are equal to its current liabilities. This also indicates that the organization can pay off its current debts without selling its long-term assets. If an organization has a quick ratio higher than 1, this means that it owns more quick assets than current liabilities.
...
* **Numerator:** Quick Assets [or cash + short-term investments + current receivables] 
  <br>
...
```{r, fig.height=4}
x.05 <- quantile( core$quick_ratio, 0.05, na.rm=T )
x.95 <- quantile( core$quick_ratio, 0.95, na.rm=T )

ggplot( core, aes(x = quick_ratio ) ) +  
...
mean( core2$quick_ratio < 0, na.rm=T ) 
core2$quick_ratio[ core2$quick_ratio < 0 ] <- 0

# proption of values above 200% 
mean( core2$quick_ratio > 50, na.rm=T ) 
core2$quick_ratio[ core2$quick_ratio > 50 ] <- 50
...
core2$quick_ratio[ core2$quick_ratio < x.05 ] <- x.05
core2$quick_ratio[ core2$quick_ratio > x.95 ] <- x.95
```
...
core2 %>%
  mutate( # quick_ratio = quick_ratio * 10000,
  ...
  stargazer( type = s.type, 
             ...
             summary.stat = c("min","p25","median",
                              "mean","p75","max", "sd"),
             covariate.labels = c("Quick Ratio", "Age", 
...
What proportion of orgs have quick ratios equal to zero?


```{r}
prop.zero <- mean( core2$quick_ratio == 0, na.rm=T )
```


In the sample, `r 100*round(prop.zero,2)` percent of the organizations have quick ratios equal to zero, meaning they have no quick cash available. These organizations are dropped from subsequent graphs to keep the visualizations clean. The interpretation of the graphics should be the distributions of quick ratios for organizations that have positive or negative values.  
...
```{r, fig.height=4}
min.x <- min( core2$quick_ratio, na.rm=T )
max.x <- max( core2$quick_ratio, na.rm=T )

ggplot( core2, aes(x = quick_ratio )) +  
  geom_density( alpha = 0.5 ) + 
  xlim( min.x, max.x  ) +
  xlab( variable.label ) +
...
ggplot( core3, aes( x=quick_ratio ) ) + 
  geom_density( alpha = 0.5) + 
  # xlim( -0.1, 1 ) +
...
jplot( log10(core3$totfuncexpns), core3$quick_ratio, 
...
jplot( log10(core3$totrevenue), core3$quick_ratio, 
...
jplot( log10(core3$totnetassetend), core3$quick_ratio, 
...
jplot( log10(core3$totassetsend), core3$quick_ratio, 
...
jplot( core3$AGE, core3$quick_ratio, 
       xlab="Nonprofit Age", 
       ylab=variable.label ) 
...
jplot( core3$lndbldgsequipend, core3$quick_ratio, 
       xlab="Land and Building Value", 
       ylab=variable.label ) 
...
```{r, eval=F}
core.quick_ratio <- select( core, ein, tax_pd, quick_ratio )
saveRDS( core.quick_ratio, "03-data-ratios/m-12-quick-ratio.rds" )
write.csv( core.quick_ratio, "03-data-ratios/m-12-quick-ratio.csv" )
```
