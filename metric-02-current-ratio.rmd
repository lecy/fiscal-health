...
A ratio below 0 is not very meaningful. Create an upper limit of 200%. 
 

```{r}
# proportion of values that are negative
mean( core$currentratio < 0, na.rm=T ) 
core$currentratio[ core$currentratio < 0 ] <- 0

# proption of values above 200% 
mean( core$currentratio > 200, na.rm=T ) 
core$currentratio[ core$currentratio > 200 ] <- 200
```
...
```{r}
prop.zero <- mean( core$currentratio == 0, na.rm=T )
```
...
```{r, fig.height=4}
min.x <- min( core$currentratio, na.rm=T )
max.x <- max( core$currentratio, na.rm=T )
